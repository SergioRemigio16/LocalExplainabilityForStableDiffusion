...
def llava_reward_qa_loss(
    pixel_values,
    prompts,
    batched_questions=[["What is the quality of this image?", "What is the main subject of this image?"]],
    batched_options=[["A. Terrible\nB. Poor\nC. Ok\nD. Good\nE. Excellent", "A. A cripsy red apple\nB. A granny smith apple\nC. A apple that is decomposing\nD. A apple filled with worms and decomposing"]],
    batched_answers=[["E", "D"]],
):

    accum_steps = len(batched_questions[0])
    for prompt, questions, answers, options in zip(
        prompts, batched_questions, batched_answers, batched_options
    ):
        prompt_d = {
            "prompt": prompt,
        ...
        loss, reward = llava_reward_qa(pixel_values, [prompt_d])
        print("got loss rfn", loss)
        yield loss, accum_steps



def call_pipe_and_get_prompt_grads(pipe, prompts, pipe_kwargs):
    out = patched_call(
        self=pipe,
        prompt=prompts,
        output_type="pt",
    ...
    # grad accumulation
    for loss, accum_steps in llava_reward_qa_loss(out.images, prompts):
        print("Got loss", loss)
...
grads, hidden_states, images = integrated_grads(pipe, [prompt],)
